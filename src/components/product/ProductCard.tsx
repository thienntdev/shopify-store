...
interface ProductCardProps {
  ...
  variant?: "default" | "compact"; // Variant cho search bar
...
export default function ProductCard({
  ...
  const contentClass = isCompact ? "" : "p-4 flex flex-col flex-1";
  ...
  const content = (
    <div className={containerClass}>
      {/* Badge - chỉ hiển thị khi không phải compact */}
      {!isCompact && shouldShowBadge && badge && (
        <div className="absolute top-2 left-2 z-10 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded">
          {badge}
        </div>
      )}

      {/* Discount Badge - chỉ hiển thị khi không phải compact */}
      ...
      <div className={contentClass}>
        ...
        {/* Rating - Chỉ hiển thị nếu có reviewCount > 0 và không phải compact */}
        {!isCompact && reviewCount > 0 && (
